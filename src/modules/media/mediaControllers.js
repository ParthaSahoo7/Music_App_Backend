...
const initiateUploadController = async (req, res) => {
  ...
  try {
    ...
    const { uploadId, key } = await mediaService.initiateUpload(userId, { filename, contentType });
...
const getPresignedUrlsController = async (req, res) => {
  ...
  try {
    ...
    const urls = await mediaService.getPresignedUrls(userId, uploadId, key, partNumbers);

    return res.status(200).json(successResponse(urls, 'Presigned URLs generated successfully.'));
  } catch (error) {
...
const abortUploadController = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json(successResponse(result, 'Multipart upload aborted successfully.'));
  } catch (error) {
...
const getMediaAccessUrlController = async (req, res) => {
  ...
  try {
    ...
    const accessUrl = await mediaService.getMediaAccessUrl(userId, mediaId, variantResolution);
...
const getAllMediaController = async (req, res) => {
  ...
  try {
    const userId = req.user.userId;
    const media = await mediaService.getAllMedia(userId, req.query);

    return res.status(200).json(successResponse(media, 'Media retrieved successfully'));
  } catch (error) {
...
const updateMediaController = async (req, res) => {
  ...
  try {
    ...
    const userId = req.user._id;
    const media = await mediaService.updateMedia(userId, req.params.id, req.body);

    return res.status(200).json(successResponse(media, 'Media updated successfully'));
  } catch (error) {
...
const deleteMediaController = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json(successResponse(result, 'Media deleted successfully'));
  } catch (error) {
